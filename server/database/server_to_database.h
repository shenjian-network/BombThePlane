...
public:
    ...
    unsigned int PushBackUserReceive(const char* uname,const char* send_time,const char* from_name,const char* to_name,const unsigned int& cid,const char& ctype);//返回插入的id值
    int GetLastTexts(const int& row_num,const char* name,const char* op_name);//返回获得的行数，其结果时间从大到小依次排列，其字段依次为（send_time,from_user,ctype,fkey,fname,mcontain）
